...
class payme {
	...
	public function create_hash($system_db,$client_db,$masterSystemObj,$payme_users_head_id){
		...
		$toEncrypt = serialize(array('id_payme'=> $payme_users_head_id));
		
		/*get IV from system*/
		$iv_user = $masterSystemObj->get_iv_user();	

		$development_crypt = new development_crypt();
		$development_crypt->encrypt($GLOBALS['key_program'],$iv_user,$toEncrypt);
		
		$encrypt_hash = rawurlencode($development_crypt->get_encrypt());
	...
	*/
	public function payme_submit($system_db,$client_db,$array_input) {
		...
		$id				= $array_input['payme_users_head_id'];
		...
		$payme_users_head->set_fk_stores($_SESSION['masterStore']);
	...
	public function payme_edit($system_db,$client_db,$payme_users_head_id=false) {
		
		$this->error = false;
		
		//print $payme_users_head_id;
		$field = array();
		...
		*/
		...
		if($payme_users_head_id){ //detect if id exist
			...
			$payme_status = $obj_payme_users_head->get_status(); //status
			...
			if($payme_status <> 'O'){ //if not Open Block
				$field[0]['readonly'] = 1;
				$field[1]['readonly'] = 1;
				$field[2]['readonly'] = 1;
			}
		...
		while($row = $system_db->get_row ( $rs )) {
			...
			$field[4]['name'][$pay_id]['name'] = "pay_type[".$obj_payme_type->get_code()."]";
			$field[4]['name'][$pay_id]['label'] = $obj_payme_type->get_name()." ";
			
			$field[4]['name'][$pay_id]['value'] = "1";
			
			/*Detect at payme_users_detail [client] if generated code is there */
			if($payme_users_head_id){
				...
				FROM 
					payme_users_detail 
				WHERE 
					payme_users_detail.fk_payme_users_head = '$payme_users_head_id' 
					AND payme_users_detail.fk_mygest_payme_type = '".$obj_payme_type->get_id()."' 
					AND payme_users_detail.type = 'HTML' 
					AND payme_users_detail.enabled = '1'
					
				");
				
				$rs_pud = $client_db->execute ( $sql_pud );
				
				$num_rows_pud = mysql_num_rows($rs_pud);
				
				if($num_rows_pud > 0){ //if exist it's at payme
					$field[4]['name'][$pay_id]['checked'] = "1";
				}
		...
		if($payme_users_head_id){ 
			$field[5]['type'] = "hidden";
			$field[5]['name'] = "payme_users_head_id";
			$field[5]['value'] = $payme_users_head_id;
		}
	...
	private function payme_payTypesConnect(){
	}
	// Destructor //
