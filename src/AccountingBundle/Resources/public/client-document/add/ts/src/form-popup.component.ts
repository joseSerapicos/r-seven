...
import {FormExtModule as ClientDocumentPaymentEditFormExtModule} from '../../../../client-document-payment/edit/ts/src/form.ext-module';
                ...
                switch (this._formService.getObject()['documentType_type']) {
                    case 'RECTIFICATION':
                        return {
                            ...
                            urlProvider: (this._helperService.getAppVar('route') + 'accounting/client-document-invoice-rectification/data-for-rectification/' + this._formService.getObject()['id'] + (bookingContext ? ('/' + bookingId) : ''))
                        };
                    case 'RECEIPT':
                    case 'SETTLEMENT':
                        return {
                            ...
                            urlProvider: (this._helperService.getAppVar('route') + 'accounting/client-document/data-for-settlement/' + this._formService.getObject()['id'] + (bookingContext ? ('/' + bookingId) : ''))
                        };
                    case 'PAYMENT':
                        return {
                            module: ClientDocumentPaymentEditFormExtModule,
                            component: 'FormComponent',
                            urlProvider: (this._helperService.getAppVar('route') + 'accounting/client-document-receipt-payment/conf/' + this._formService.getObject()['id'])
                        };
                    default: // INVOICE
                        return {
                            module: TreeViewExtModule,
                            component: 'TreeViewComponent',
                            urlProvider: (this._helperService.getAppVar('route') + 'booking/booking-service-price/data-for-invoice-client/' + this._formService.getObject()['clientObj'] + (bookingContext ? ('/' + bookingId) : ''))
                        };
                }
            case 2:
                switch (this._formService.getObject()['documentType_type']) {
