...
namespace Frt\BookingBundle\Controller;

use Sensio\Bundle\FrameworkExtraBundle\Configuration\Route;
use Symfony\Component\HttpFoundation\Request;

class PackageBookingController extends \Bck\BookingBundle\Controller\PackageBookingController
{
    /**
    ...
    /**
     * @Route("/booking/package-booking/detail-embed/{system}/{id}",
     *     name="_booking__package_booking__detail_embed",
     *     defaults={"id" = null},
     * )
     *
     * Detail action
     ...
     * @param $id (this parameter is mandatory, but when the conf is set there is not any id to generate the route,
     * the id is provided at runtime so this requires a default value in the route definition)
     * @param $system (system to load the local database)
     * @return mixed
     * @throws \Exception
     */
    public function detailEmbedAction(Request $request, $system, $id)
    {

        //isto deveria ser o package service!!!!!!!!!!!!!!!!!
    //to get the package full info!!!!!
        ...
        $this->get('app.service.frt')->loadSystem($system);

        // Set configuration
        $this->init($request);
        $this->localConf['templates']['detail'] = 'FrtBookingBundle:PackageBooking:detail-embed.html.twig';
        ...
        return $this->detailAction($request, $id);
    }
    ...
    /**
     * @Route("/booking/package-booking/add-embed/{system}/{id}",
     *     name="_booking__package_booking__add_embed",
     *     defaults={"id" = null}
     * )
     *
     * Action to add objects using the form
     * @param Request $request
     * @param $system (system to load the local database)
     * @param $packageService (used when return to the first step)
     * @return mixed
     * @throws \Exception
     */
    public function addEmbedAction(Request $request, $system, $packageService)
    {
        if (!$system) {
            throw new \Exception('Invalid request parameters.');
        }
        ...
        $packageServiceObj = $packageService;
        ...
        // Set system
        return $this->addStep1Action($request, $obj->getId());
    }
