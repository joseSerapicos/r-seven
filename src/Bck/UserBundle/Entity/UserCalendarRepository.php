...
class UserCalendarRepository extends BaseEntityRepository {
    ...
    static function getMetadata()
    {
        ...
        return self::$metadata = self::processMetadata(array(
            ...
            'shareUserObj' => array('label' => 'User to Share', 'type' => 'object', 'acl' => 'edit', 'typeDetail' => array(
                'table' => 'user', 'bundlePrefix' => '', 'bundle' => 'login', 'type' => 'select'), 'isRequired' => false),
            'userGroupObj' => array('label' => 'Group to Share', 'type' => 'object', 'acl' => 'edit', 'typeDetail' => array(
                'table' => 'userGroup', 'bundlePrefix' => 'bck', 'bundle' => 'user', 'type' => 'select'), 'isRequired' => false),
            'storeObj' => array('label' => 'Store to Share', 'type' => 'object', 'acl' => 'edit', 'typeDetail' => array(
                'table' => 'store', 'bundlePrefix' => 'bck', 'bundle' => 'admin', 'type' => 'select'), 'isRequired' => false),
    ...
    public function getLocalObjects($options, $isAdmin, $loggedUserId)
    {
        ...
        if (!$isAdmin) { // Admin has access all stores
            ...
            $qb->leftJoin('Bck\AdminBundle\Entity\Store',
                'store'
            );*/
            $qb->leftJoin('Bck\AdminBundle\Entity\UserGroupAcl', // Groups Acl of store
                'userGroupAcl',
                'WITH',
                ('userGroupAcl.isEnabled = 1 AND store.isEnabled = 1 AND userGroupAcl.storeObj = store.id')
            );
            $qb->leftJoin('Bck\AdminBundle\Entity\UserGroupAclUser', // Groups Acl of logged user
                'userGroupAclUser',
                'WITH',
        ...
        // Filter by group
        /* Already defined in $options
        $qb->leftJoin('Bck\UserBundle\Entity\UserGroup',
            'userGroup'
        );*/
        $qb->leftJoin('Bck\UserBundle\Entity\UserGroupUser', // Groups where logged user belongs
            'userGroupUser',
            'WITH',
