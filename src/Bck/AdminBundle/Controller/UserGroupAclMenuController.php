...
use Bck\AdminBundle\Entity\UserGroupAcl;
...
class UserGroupAclMenuController extends BaseEntityChildController
{
     ...
     * @return mixed
     */
    static function getLabel() { return 'ACL'; }
    ...
    public function init(Request $request, $parents = null)
    {
        ...
        $this->templateConf['route'] = array(
            'get' => array(
                'name' => '_bck__admin__user_group_acl_menu__get'
            ),
            'edit' => array(
                'name' => '_bck__admin__user_group_acl_menu__edit',
            )
        );
    ...
    /**
     * @Route("/bck/admin/user-group-acl-menu/get/{userGroupAcl}/{id}",
     *     name="_bck__admin__user_group_acl_menu__get",
     *     defaults={"id" = null}
     * )
     *
    ...
    /**
     * @Route("/bck/admin/user-group-acl-menu/edit/{userGroupAcl}",
     *     name="_bck__admin__user_group_acl_menu__edit"
     * )
     *
     * Overrides parent method
    ...
    public function editLocalChildAction(Request $request, $userGroupAcl)
    {
        ...
        $this->init($request, array($userGroupAcl));
        ...
        if($form->isSubmitted()) {
            ...
            if (!empty($data['acl']) && is_array($data['acl'])) {
                foreach ($data['acl'] as $menu => $acl) {
                    // Get menu object
                    $moduleMenuObj = $this->getRepositoryService('ModuleMenu', 'AdminBundle', 'Bck')
                        ->execute(
                            'findOneById',
                            array($menu)
                        );
    ...
    protected function getObjectsBySearch() {
        ...
        $userGroupAclObjArr = $this->get('bck.admin.service.repository')
