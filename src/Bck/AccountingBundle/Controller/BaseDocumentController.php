...
abstract class BaseDocumentController extends BaseEntityController
{
    ...
    public function addStep1Action(Request $request, $id)
    {
        ...
        $this->init($request);
        $this->localConf['formTypeClass'] = ('Bck\AccountingBundle\Form\\' . $entityContextUC . 'DocumentAddStep1Type');
    ...
    public function addStep2Action(Request $request, $id)
    {
        ...
        $options = ((empty($data['id']) || !is_array($data['id']))
            ? null
            : array('criteria' => array(array('field' => 'id', 'expr' => 'in', 'value' => $data['id'])))
        );
    ...
    public function addStep3InvoiceAction(Request $request, $id)
    {
        ...
        $this->flags['hasForm'] = true;
        $this->init($request);
        $this->localConf['formTypeClass'] = ('Bck\AccountingBundle\Form\\' . ucfirst($entityContext) . 'DocumentAddStep3InvoiceType');
        ...
        $this->localConf['templates']['edit'] = 'BckAccountingBundle:BaseDocument:add-invoice-detail.html.twig';
    ...
    public function addStep3ReceiptAction(Request $request, $id)
    {
        ...
        // Build form
        $form = $this->createForm('Bck\AccountingBundle\Form\\' . ucfirst($entityContext) . 'DocumentAddStep3ReceiptType', $obj);
        ...
        if($form->isSubmitted()) {
            ...
            // Save object in entity manager
            $this->flags['storage'] = 'db'; // Switch storage to database
            $clonedObj = clone $obj; // Clone object to save in database
            $this->saveForm($form, $clonedObj, true, false); // Doesn't flush data until all objects are inserted
            ...
            foreach ($detailObjects as $detailObj) {
                ...
                $clonedDetailObj = clone $detailObj; // Clone object to save in database
                $clonedDetailObj->setClientDocumentObj($clonedObj);
                parent::saveObject($clonedDetailObj, false); // Doesn't flush data until all objects are inserted
            }

            // Flush (persist) all objects in database
            $this->flushEm();
            $this->postSaveObject($clonedObj);

            if($this->responseConf['status'] == 1) {
                $this->updateDocumentsRemainSettlement($clonedObj);
                ...
                // Config response
                if ($this->responseConf['hasObject']) {
                    $this->responseConf['object'] = $this->normalizeObject($clonedObj); // Object updated
                }
        ...
        // Render form
        return $this->render('BckAccountingBundle:BaseDocument:add-receipt-detail.html.twig', array(
            '_conf' => $this->templateConf,
            '_form' => $form->createView()
        ));
    }
    ...
    public function addStep3PaymentAction(Request $request, $id)
    {
        ...
        // Build form
        $form = $this->createForm('Bck\AccountingBundle\Form\\' . ucfirst($entityContext) . 'DocumentAddStep3PaymentType', $obj);
        ...
        // Render form
        return $this->render('BckAccountingBundle:BaseDocument:add-receipt-payment.html.twig', array(
            '_conf' => $this->templateConf,
            '_form' => $form->createView()
        ));
    }
    ...
    public function addStep3SettlementAction(Request $request, $id)
    {
        ...
        $this->flags['hasForm'] = true;
        $this->init($request);
        $this->localConf['formTypeClass'] = ('Bck\AccountingBundle\Form\\' . ucfirst($entityContext) . 'DocumentAddStep3ReceiptType');
        // Field to be rendered in form does not necessary, because we will render a specific form
        // (this configuration to Angular "FormService" is defined in "provider")
        ...
        $this->localConf['templates']['edit'] = 'BckAccountingBundle:BaseDocument:add-receipt-settlement.html.twig';
    ...
    public function editInvoiceAction(Request $request, $id)
    {
        ...
        $this->flags['hasForm'] = true;
        $this->init($request);
        $this->localConf['formTypeClass'] = ('Bck\AccountingBundle\Form\\' . ucfirst($entityContext) . 'DocumentInvoiceType');
        // Field to be rendered in form does not necessary, because we will render a specific form
        // (this configuration to Angular "FormService" is defined in "provider")
        ...
        // Render form
        return $this->render('BckAccountingBundle:BaseDocument:edit-invoice.html.twig', array(
            '_conf' => $this->templateConf,
            '_form' => $form->createView()
        ));
    }
    ...
    public function editReceiptAction(Request $request, $id)
    {
        ...
        // Build form
        $form = $this->createForm('Bck\AccountingBundle\Form\\' . ucfirst($entityContext) . 'DocumentReceiptType', $obj);
        ...
        // Render form
        return $this->render('BckAccountingBundle:BaseDocument:edit-receipt.html.twig', array(
            '_conf' => $this->templateConf,
            '_form' => $form->createView()
        ));
    }
    ...
    public function editPaymentAction(Request $request, $id)
    {
        ...
        // Build form
        $form = $this->createForm('Bck\AccountingBundle\Form\\' . ucfirst($entityContext) . 'DocumentReceiptReceiptPaymentType', $obj);
        ...
        // Render form
        return $this->render('BckAccountingBundle:BaseDocument:edit-receipt-payment.html.twig', array(
            '_conf' => $this->templateConf,
            '_form' => $form->createView()
        ));
    }
    ...
    public function editSettlementAction(Request $request, $id)
    {
        ...
        $this->flags['hasForm'] = true;
        $this->init($request);
        $this->localConf['formTypeClass'] = ('Bck\AccountingBundle\Form\\' . ucfirst($entityContext) . 'DocumentReceiptType');
        ...
        // Render form
        return $this->render('BckAccountingBundle:BaseDocument:edit-receipt-settlement.html.twig', array(
            '_conf' => $this->templateConf,
            '_form' => $form->createView()
        ));
    }
    ...
    public function editEntityAddressAction(Request $request, $id)
    {
        ...
        $data = $this->getAndProcessRequestData($request);
        $address_id = ((isset($data) && isset($data['entityAddressObj']))
            ? $data['entityAddressObj']
            : null
        );
    ...
    {
        return $this->preCancelObject($object);
    }
    ...
    protected function preCancelObject($object) {
        if(!$this->validateDocumentAclByFormContext($this, $object, 'cancel')) {
            return false;
        }
        ...
        switch ($documentType) {
            ...
            case 'PAYMENT':
                ...
                if ($object->getTotal() != $object->getRemainSettlement()) {
                    $errorMessage = (
                        'Document has settlements ('
                        . ($object->getTotal() - $object->getRemainSettlement())
    ...
    protected function postCancelObject($object)
    {
        if ($this->responseConf['status'] == 1) {
            ...
            switch ($documentType) {
                case 'INVOICE':
                case 'RECTIFICATION':
                    $this->updateBookingInvoiceStatus($object);
                    break;
                case 'RECEIPT':
                case 'SETTLEMENT':
                    $this->updateDocumentsRemainSettlement($object);
                    break;
            }
        }
    ...
    public function pdfAction(Request $request, $id)
    {
        ...
        // Get html
        $view = 'BckAccountingBundle:BaseDocument:pdf/base.html.twig';
        $dependenciesData = array();
        switch ($object->$getMethod()->getType()) {
            case 'INVOICE':
            case 'RECTIFICATION':
                $view = 'BckAccountingBundle:BaseDocument:pdf/invoice.html.twig';
                $dependencyData = $this->forward('BckAccountingBundle:'.$entityContextUC.'DocumentInvoiceDetail:dataLocalChild',
                    array(
                        'request' => $request,
                ...
                break;
            case 'RECEIPT':
                $view = 'BckAccountingBundle:BaseDocument:pdf/receipt.html.twig';
                // Settlement
                $dependencyData = $this->forward('BckAccountingBundle:'.$entityContextUC.'DocumentReceiptSettlement:dataLocalChild',
                    array(
                        'request' => $request,
                        ($entityContext.'Document') => $object->getId(),
                        'responseType' => 'array'
                ...
                // Payment
                $dependencyData = $this->forward('BckAccountingBundle:'.$entityContextUC.'DocumentReceiptPayment:dataLocalChild',
                    array(
                        'request' => $request,
                        ($entityContext.'Document') => $object->getId(),
                        'responseType' => 'array'
                ...
                break;
            case 'PAYMENT':
                $view = 'BckAccountingBundle:BaseDocument:pdf/payment.html.twig';
                $dependencyData = $this->forward('BckAccountingBundle:'.$entityContextUC.'DocumentReceiptPayment:dataLocalChild',
                    array(
                        'request' => $request,
                        ($entityContext.'Document') => $object->getId(),
                ...
                break;
            case 'SETTLEMENT':
                $view = 'BckAccountingBundle:BaseDocument:pdf/settlement.html.twig';
                $dependencyData = $this->forward('BckAccountingBundle:'.$entityContextUC.'DocumentReceiptSettlement:dataLocalChild',
                    array(
                        'request' => $request,
                        ($entityContext.'Document') => $object->getId(),
        ...
        //$data['_localData']['data']['block'] = 'header'; // Use for debug in browser
     ...
     * @return mixed
     */
    public function setResponseStoreInfo($storeObj) {
        $response = array();

        // Set store optional info in local data (this info is not saved in document, is dynamic)
        $response['storeInfo'] = array_merge(
            $this->getRepositoryService('StorePhone', 'AdminBundle', 'Bck')
                ->execute('getForDocumentsByStore', array($storeObj)),
            $this->getRepositoryService('StoreEmail', 'AdminBundle', 'Bck')
                ->execute('getForDocumentsByStore', array($storeObj)),
            $this->getRepositoryService('StoreLink', 'AdminBundle', 'Bck')
                ->execute('getForDocumentsByStore', array($storeObj))
        );

        // @TODO: get from session
        $storeLogoImageObj = $this->getRepositoryService('StoreLogoImage', 'AdminBundle', 'Bck')
            ->execute(
                'findOneBy',
                array(array(
                    'storeObj' => $storeObj,
        ...
        $this->templateConf['localData']['data'] = array_merge(
            $this->templateConf['localData']['data'],
            $response
        );
     ...
     * @return mixed
     */
    public function setResponseDocumentInfo($object) {
        $response = array();
        ...
        $getMethod = ('get' . ucfirst($this->getEntityContext()) . 'DocumentTypeObj');
        $documentTypeSettingObj = $this->getRepositoryService($entityContextUC.'DocumentTypeSetting', 'AccountingBundle', 'Bck')
            ->execute(
                'findOneBy',
                array(array(
            ...
            if (count($bookingArr) > 0) {
                ...
                foreach ($bookingArr as $booking) {
                    if ($booking['bookingObj']) {
                        ...
                        $bookingObjArr[$booking['bookingObj']] = $this->getRepositoryService('Booking', 'BookingBundle', 'Bck')
                            ->execute(
                                'findOneById',
    ...
    protected function updateDocumentsRemainSettlement($object) {
        ...
        $documentReceiptSettlementObjArr = $this->getRepositoryService($entityContextUC . 'DocumentReceiptSettlement', 'AccountingBundle', 'Bck')
            ->execute(
     ...
     * @return mixed
     */
    public function setResponseDocumentTotalVatSplitByCode($object) {
        $response = array();
        ...
        switch ($this->flags['storage']) {
            case 'session':
                ...
                $response['totalVatSplitByCode'] = array_values($totalVatSplitByCode);
                break;
            default:
                $response['totalVatSplitByCode']
                    = $this->getRepositoryService($entityContextUC . 'DocumentInvoiceDetail', 'AccountingBundle', 'Bck')
                    ->execute(
    ...
    protected function addDocumentReceiptDetail(&$object, &$detailObjects)
    {
        ...
        // Controls payment entries, to avoid repeat the same ClientPaymentRequest
        // (otherwise the user can pay with the same ClientPaymentRequest multiple times in the same payment
        // and create an inquiry payment)
        $clientPaymentRequestIds = array();
        foreach ($paymentObjects as $paymentObject) {
            ...
            // Check if invoice document is already added
            if ($clientPaymentRequestObj = $paymentObject->getClientPaymentRequestObj()) {
                $clientPaymentRequestId = $clientPaymentRequestObj->getId();
                if (in_array($clientPaymentRequestId, $clientPaymentRequestIds)) {
                    $this->responseConf['status'] = 0;
                    $this->addFlashMessage(
                        'Payment (' . $clientPaymentRequestObj->getDescription() . ') duplication not allowed.',
                        'Data not persisted',
                        'error'
                    );
                ...
                $clientPaymentRequestIds[] = $clientPaymentRequestId;
            }
    ...
    protected function initForSettlement(Request $request, $targetDocument, $booking = null)
    {
        ...
        $this->templateConf['route'] = array(
            'get' => array(
                'name' => '_bck__accounting__'.$entityContext.'_document__get_for_settlement',
                'url' => $this->generateUrl(
                    '_bck__accounting__'.$entityContext.'_document__get_for_settlement',
    ...
    static function getObjectsBySearchForSettlement($controller, $options = array(), $targetDocument = null)
    {
        ...
        $objects = $controller->getRepositoryService($entityContextUC.'Document', 'AccountingBundle', 'Bck')
            ->execute(
                'getForSettlement',
