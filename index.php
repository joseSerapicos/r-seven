...
<body>
		...
		<div class="container">
			...
			<div class="collapse navbar-collapse navbar-ex1-collapse">
				...
				<ul class="nav navbar-nav navbar-right">
					...
					<li class="dropdown">
                    	<a href="javascript:;" class="dropdown-toggle" data-toggle="dropdown_index_page"> <i class="icon-user"></i> <?=$masterUserObj->get_name ()?> <b class="caret"></b></a>
						<ul class="dropdown-menu">
							<!-- <li class="divider"></li> -->
			...
			<div class="container">
				...
				<div class="collapse subnav-collapse">
					<ul class="mainnav">
						<?php
						$allModules = false;
						$allModules = $masterMainObj->get_modules();

						if (is_array($allModules))
							foreach ($allModules as $module) {
								if($module->get_has_menus($masterMainDb)) { ?>
									<li id="menu_<?= $module->get_id() ?>" class="dropdown <?= (($module->get_id()==$masterModule)?'active':'') ?>">
										...
										<?php
										$allMenus = $masterMainObj->get_moduleMenus($module->get_id());
								...
								<?php
									...
									$menu = false;
									$allMenus = $masterMainObj->get_moduleMenus($module->get_id());
									if(is_array($allMenus)) $menu = $allMenus[0];
									 ?>
									<li id="menu_<?= $module->get_id() ?>" <?= (($module->get_id()==$masterModule)?'class="active"':'') ?>>
                                    	<a href='javascript: setMenu(<?= json_encode(array("module" => $module->get_id(), "menu" => ($menu?$menu->get_id():""))) ?>);'>
                                        	<i class="<?= $module->get_icon($masterMainDb) ?>"></i>
                                        	<span><?= $module->get_name() ?></span>
										</a>
                            		</li>
	...
	<?php
    $include_file = ($masterMenuObj?($masterMenuObj->get_filepath_top($masterMainDb)):($masterModuleObj?($masterModuleObj->get_filepath_top($masterMainDb)):(false)));
	if($include_file) {
    ...
    // Bottom do menu
    $include_file = ($masterMenuObj?($masterMenuObj->get_filepath_bottom($masterMainDb)):($masterModuleObj?($masterModuleObj->get_filepath_bottom($masterMainDb)):(false)));
    if($include_file) {
		if($include_file == 'default') {
