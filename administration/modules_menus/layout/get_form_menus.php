        ...
        <div class="col-md-8">
            <select id="fk_mygest_modules_menus_info" name="fk_mygest_modules_menus_info" class="form-control">
    			...
    			foreach ( $modulesMenusInfo as $moduleMenuInfo ) { ?>
                	<option value="<?= $moduleMenuInfo->get_id() ?>" <?= ($obj?(($obj->get_fk_mygest_modules_info()==$moduleMenuInfo->get_id())?('selected="selected"'):''):'') ?> data-myPriority="<?= $moduleMenuInfo->get_priority() ?>" data-myName="<?= $moduleMenuInfo->get_name() ?>"><?= $moduleMenuInfo->get_name() ?></option>
                <?php } ?>
            </select>
        </div>
    </div> <!-- /.form-group -->
