...
if(!empty($_POST['json_post']))
{
	...
	if(is_object($post))
	{
		$obj = new development_file_system($post->id);
...
<form role="form" class="form-horizontal col-md-7">
    ...
    <div class="form-group ">
        <label for="fk_development_file_system" class="col-md-4">Parent</label>
        <div class="col-md-8">
            <select id="fk_development_file_system" name="fk_development_file_system" class="form-control">
                <option value="0"></option>
                <?php if(is_array($file_system_array))
				foreach($file_system_array as $file_system) { ?>
                    <option value="<?= $file_system->get_id() ?>" <?= ($obj?(($obj->get_fk_development_file_system()==$file_system->get_id())?('selected="selected"'):''):'') ?>><?= $file_system->get_name() ?></option>
                <?php } ?>
            </select>
        </div>
      </div> <!-- /.form-group -->
