					...
					<div class="widget-content">
                        ...
                        if (is_array ( $framework_array ))
                            foreach ( $framework_array as $framework ) { ?>
                                <button type="button" class="btn btn-default" onclick='javascript: clear_framework_interface_trash(); call_script("<?= PATH_ROOT.$framework->get_link() ?>", "", "tag", "#load_menus");'><?= $framework->get_name() ?></button>
                        <?php } ?>	        
                        <!-- /Menus da framework -->
                    </div>
                </div>
