...
class Pdf_Table_Cell_Image extends Pdf_Table_Cell_Abstract implements Pdf_Table_Cell_Interface {
     ...
     *
     * @param Pdf Object:Pdf Interface $pdf
     * @param string $file
     * @param number $width
     * @param number $height
     * @param string $type
    ...
    public function render () {
        ...
        $x = $this->oPdf->GetX() + $this->BORDER_SIZE;
        $y = $this->oPdf->GetY() + $this->BORDER_SIZE;
        ...
        if ( strpos( $this->sAlignment, 'J' ) !== false ) {
            ...
            $x += $this->PADDING_LEFT;
            $this->setContentWidth( $this->getCellDrawWidth() - 2 * $this->BORDER_SIZE - $this->PADDING_LEFT - $this->PADDING_RIGHT );
            ...
            //right
            $x += $this->getCellDrawWidth() - $this->getContentWidth() - $this->PADDING_RIGHT;
        } else {
        ...
        if ( strpos( $this->sAlignment, 'T' ) !== false ) {
            ...
            //bottom
            $y += $this->getCellDrawHeight() - $this->getContentHeight() - $this->PADDING_BOTTOM;
        } else {
    ...
    public function processContent () {
        ...
        $this->setCellHeight( $this->getContentHeight() + $this->PADDING_TOP + $this->PADDING_BOTTOM + 2 * $this->BORDER_SIZE );
